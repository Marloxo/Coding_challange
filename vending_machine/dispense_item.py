...
class DispenseItem(State):
    ...
    def display_stock(self) -> None:
        logging.critical(f"Can't display stock in dispense Item state")


    def select_item(self, product_code) -> None:
        logging.critical(f"Can't select item in dispense Item state")


    def collect_cash(self, amount) -> None:
        logging.critical(f"Can't collect cash in dispense Item state")
    ...
    def dispense_change(self) -> None:
        logging.critical(f"Can't dispense change in dispense Item state")


    def cancel_transaction(self) -> None:
        logging.critical(f"Can't cancel transaction in dispense Item state")
