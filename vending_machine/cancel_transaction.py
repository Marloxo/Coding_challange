...
class CancelTransaction(State):
    ...
    def display_stock(self) -> None:
        logging.critical(f"Can't display stock in a cancelled transaction")


    def select_item(self, product_code) -> None:
        logging.critical(f"Can't select item in a cancelled transaction")


    def collect_cash(self, amount) -> None:
        logging.critical(f"Can't collect cash in a cancelled transaction")


    def dispense_item(self) -> None:
        logging.critical(f"Can't dispense item in a cancelled transaction")


    def dispense_change(self) -> None:
        logging.critical(f"Can't dispense change in a cancelled transaction")
